...
#define VL53L0X_startTimeout() (timeout_start_ms = pa_millis())

// Check if timeout is enabled (set to nonzero value) and has expired
#define VL53L0X_checkTimeoutExpired() (io_timeout > 0 && ((uint16_t)(pa_millis() - timeout_start_ms) > io_timeout))
...
VL53L0X VL53L0X::instance =VL53L0X();
// Public Methods //////////////////////////////////////////////////////////////
...
void VL53L0X::writeReg(uint8_t reg, uint8_t value)
{
  ...
  pa_IIC_writeLen(address, reg, 1, &value, a);
...
void VL53L0X::writeReg16Bit(uint8_t reg, uint16_t value)
{
  ...
  data[0] = ((value >> 8) & 0xFF);
  data[1] = (value & 0xFF);
  pa_IIC_writeLen(address, reg, 2, data, a);
...
void VL53L0X::writeReg32Bit(uint8_t reg, uint32_t value)
{
  ...
  data[2] = ((value >> 8) & 0xFF);
  data[3] = (value & 0xFF);
  pa_IIC_writeLen(address, reg, 4, data, a);
  // data[0]=
  // bus->beginTransmission(address);
  // bus->write(reg);
...
uint8_t VL53L0X::readReg(uint8_t reg)
{
  pa_IICSettingStruct a;
  uint8_t value;
  pa_IIC_readLen(address,reg,1,&value,a);
  // bus->beginTransmission(address);
  // bus->write(reg);
  // last_status = bus->endTransmission();
...
uint16_t VL53L0X::readReg16Bit(uint8_t reg)
{
  ...
  pa_IIC_readLen(address,reg,2,buf,a);
...
uint32_t VL53L0X::readReg32Bit(uint8_t reg)
{
  ...
  pa_IIC_readLen(address,reg,4,buf,a);
  // bus->beginTransmission(address);
  // bus->write(reg);
  // last_status = bus->endTransmission();
...
{
  pa_IICSettingStruct a;
  pa_IIC_writeLen(address,reg,count,(uint8_t *)src,a);
  // bus->beginTransmission(address);
  // bus->write(reg);
...
{
  pa_IICSettingStruct a;
  pa_IIC_readLen(address,reg,count,dst,a);
  // bus->beginTransmission(address);
  // bus->write(reg);
  // last_status = bus->endTransmission();
...
uint8_t VL53L0X::getVcselPulsePeriod(vcselPeriodType type)
{
  if (type == VcselPeriodPreRange)
  {
    return VL53L0X_decodeVcselPeriod(readReg(PRE_RANGE_CONFIG_VCSEL_PERIOD));
  }
  else if (type == VcselPeriodFinalRange)
  {
    return VL53L0X_decodeVcselPeriod(readReg(FINAL_RANGE_CONFIG_VCSEL_PERIOD));
  }
  else
  {
    return 255;
